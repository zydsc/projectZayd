...
-- Deklarasi tabel global untuk saling berbagi data 'action' antar bot
_G.bot_actions = _G.bot_actions or {}
...
function powershell(text, curent, textWorld)
    ...
    -- Simpan action bot yang sedang berjalan ke dalam tabel global
    _G.bot_actions[getBot().name] = text

    -- Susun teks informasi semua bot ke dalam satu string panjang
    local allBotInfo = ""
    for _, b in pairs(getBots()) do
        local statusBot = getBotStatus(b.status)
        local actionBot = _G.bot_actions[b.name] or "Standby / Menunggu giliran"
        
        -- Format baris: NamaBot | Status | Action
        allBotInfo = allBotInfo .. "**" .. b.name .. "** | `" .. statusBot .. "` | *" .. actionBot .. "*\n"
    end
    ...
    -- Menggabungkan Nama, Status, dan Action dalam 1 field agar tidak berantakan di HP
    wh.embed1:addField("<:bot:1119535018608435301> BOTS INFORMATION (BOT | STATUS | ACTION)", allBotInfo, false)
        ...
        for x = startX, endX, stepX do
            ...
            if getTile(x, y).fg == 0 then
                ...
                while getTile(x, y).fg == 0 do
                    ...
                    -- [PERBAIKAN]: Cek apakah dirt habis saat di dalam loop
                    if getBot():getInventory():findItem(2) == 0 then
                        print(getBot().name .. " - Kehabisan dirt, mengambil suplai baru...")
                        plantHT(world, false)
                        reconnect(world, edit_doorid)
                    end
                    -- [AKHIR PERBAIKAN]
    ...
    if auto_find_world then
        ...
        for _, world in pairs(list_world_DF) do
            ...
            if join(world, edit_doorid) then
                ...
                if total_bot < limit_bot_per_world or (total_bot == limit_bot_per_world and getIndexBots() == 1) then
                    time =  os.time()
                    reconnect(world, edit_doorid)
